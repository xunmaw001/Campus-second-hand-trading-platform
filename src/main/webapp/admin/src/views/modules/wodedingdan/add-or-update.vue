...
export default {
  ...
  computed: {
    zongjia:{
      get: function () {
        return 1*this.ruleForm.shuliang*this.ruleForm.ershoujia
      }
    },
  },
  created() {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.jiaoyifangshiOptions = "线上交易,线下交易".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `wodedingdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wodedingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wodedingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
